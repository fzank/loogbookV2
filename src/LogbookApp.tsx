...
export default function LogbookApp() {
  ...
  const alterarTipoAlvoPadrao = (tipo: 'circular' | 'humanoide' | 'duplo') => {
    setTipoAlvoPadrao(tipo);
    const novoAlvo = tipo === 'circular' ? ALVO_CIRCULAR : tipo === 'humanoide' ? ALVO_HUMANOIDE : ALVO_DUPLO;
    setImagemAlvo(novoAlvo);
    setMarcacoes([]);
    setQtdTiros('');
  };
  ...
  const editarSessao = (sessao: any) => {
    ...
    // Identifica e mantém o alvo correto da sessão ao editar
    const imgSalva = sessao.imagemOriginal || ALVO_CIRCULAR;
    setImagemAlvo(imgSalva);
    if (imgSalva === ALVO_CIRCULAR) setTipoAlvoPadrao('circular');
    else if (imgSalva === ALVO_HUMANOIDE) setTipoAlvoPadrao('humanoide');
    else if (imgSalva === ALVO_DUPLO) setTipoAlvoPadrao('duplo');
          ...
          {(!imagemAlvo || imagemAlvo === ALVO_CIRCULAR || imagemAlvo === ALVO_HUMANOIDE || imagemAlvo === ALVO_DUPLO) && (
            <div style={{marginBottom: '10px', backgroundColor: theme.cardRelatorioBg, padding: '8px', borderRadius: '6px'}}>
              ...
              <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap'}}>
                <label style={{fontSize: '12px', cursor: 'pointer'}}><input type="radio" name="alvo" checked={tipoAlvoPadrao === 'circular'} onChange={() => alterarTipoAlvoPadrao('circular')} /> Circular</label>
                <label style={{fontSize: '12px', cursor: 'pointer'}}><input type="radio" name="alvo" checked={tipoAlvoPadrao === 'humanoide'} onChange={() => alterarTipoAlvoPadrao('humanoide')} /> Humanoide</label>
                <label style={{fontSize: '12px', cursor: 'pointer'}}><input type="radio" name="alvo" checked={tipoAlvoPadrao === 'duplo'} onChange={() => alterarTipoAlvoPadrao('duplo')} /> Duplo</label>
              </div>
            </div>
          )}
